...
public class MiracleEffect implements ISpellEffect {
	...
	public void apply(SpellEffectContext context) {
		...
		GameObjectFilter isdead = new GameObjectFilter() {
			public boolean test(GameObject go) {
				return new CharacterWrapper(go).isDead();
			}
		};

		GameObject deadBuddy = SpellUtility.findNativeFromTheseGroups(nativeGroup, isdead, context.Game);

		if (deadBuddy != null) {
			SetupCardUtility.resetDenizen(deadBuddy);
